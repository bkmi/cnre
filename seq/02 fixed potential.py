...
class Ratio(torch.nn.Module):
    def __init__(self, likelihood, d, device):
        super().__init__()
...
def main():
    ...
    starting_theta = prior.sample((n,))

    ratio_fixed = Ratio(likelihood, d, "cpu")

    p = create_chain(starting_theta, ratio_fixed, prior)

    # print(starting_theta)

    samples = p.sample((20,), x=x_o)

    print(samples)

    # print(torch.isclose(samples, starting_theta.expand(*samples.shape)))
